...
/*   Updated: 2024/06/24 16:37:56 by ali-akouhar      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    pthread_mmutex_lock(&philo->get_status_lock);
    if (status == DIED)
        philo->data->death_flag = 0;
    philo->status = status;
    pthread_mmutex_unlock(&philo->get_status_lock);
}

void    ft_printf(char *str, t_philo *philo)
{
    pthread_mutex_lock(&philo->write_lock);
    printf("%lu   %i %s", (get_time() - philo->start), philo->id, str);
